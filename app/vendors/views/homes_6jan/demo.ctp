...
.containerMain{float:left;padding-bottom:20px;border-bottom: 1px dotted;border-top: 1px dotted;width:920px;}
...
<script>
   ...
   $(document).ready(function() {
    ...
    $('#tradeForm').ajaxForm({
		...
		var price  = quantity*(exitPrice-enterPrice);
	  

		$('#profitReplace').html('$'+price);
		$('#symbolsReplace').html(symbol);
		$('#entryPriceReplace').html(enterPrice);
		$('#exitPriceReplace').html(exitPrice);
...
<script>
	...
	<h1>User Trade Details</h1>
	
	
		<div class="containerMain">
			<span class="container">
				<p class="lableTrade">Symbols:</p>
				<p class="valueTrade" id="symbolsReplace"><?php echo '--';?></p>
			</span>
			<span class="container">
				<p class="lableTrade">Approximate Profit</p>
				<p class="valueTrade" id="profitReplace">$0</p>
			</span>
			<span class="container">
				<p class="lableTrade">Entry Price:</p>
				
				<p class="valueTrade" id="entryPriceReplace"><?php echo '--';?></p>
			</span>
			<span class="container">
				<p class="lableTrade">Exit Price:</p>
				<p class="valueTrade" id="exitPriceReplace"><?php echo '--';?></p>
			</span>
		</div>
		...
		<?php echo $form->create('Home',array('id'=>'tradeForm','controller'=>'homes','action'=>'checkTradeDetails','name'=>'submittrade')); ?>
			<div class="inputMainClass">
				 <input type="text"  value="Company Symbol" id="enterSymbol"  onclick="this.value='';" name="data[Home][symbol]" onblur="this.value=!this.value?'Company Symbol ':this.value;"/>
				 <input type="text"  value="Price Per Share" id="enterPrice"   onclick="this.value='';" name="data[Home][price]" onblur="this.value=!this.value?'Price Per Share':this.value;"/>
		 	
	    			<input type="text"  value="Quantity" id="quantity" onclick="this.value='';" name="data[Home][quantity]" onblur="this.value=!this.value?'Quantity':this.value;"/>
				...
				<input type="text"  value="Exit Price" id="exitPrice" onclick="this.value='';" name="data[Home][exit_price]" onblur="this.value=!this.value?'Exit Price ':this.value;"/>
