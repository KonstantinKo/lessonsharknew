...
function contentCredential($id=null)
{
 ...
 function editTeacher()
	...
	function addStudent()
	{
		...
		{
		 $id=14;	
		 $url = 'http://www.lessonshark.com/lessonshark1/students';
		 $to  = $_POST['email'];
		 $message = file_get_contents(VIEWS . 'elements' . DS . 'email' . DS . 'student_contact.ctp');
                 $message 	 = str_replace('[firstname]',$_POST['fee'],$message);
		 ...
		 if($sid)
		  {
			...
			echo '<div id="detail">';
		 	foreach($user1 as $us)
			{
				echo '<div class="student_tab_middle">
		 <div class="student_tab_edit_btn"><a href="#" onclick="getdetail()" ><img src="/dev1/img/red_edit_btn.png"></a></div>
	<div class="student_inner_lesson_main">
    <div class="student_inner_lesson_left">';
       echo '<div class="student_iiner_name_text">Name</div>
       <div class="student_inner_main">
            <div class="student_left_iiner">First Name</div>';
            echo '<div class="student_right_iiner">'  .$us['User']['firstname']. '</div>';
        echo '</div>
        <div class="student_inner_main">';
           echo '<div class="student_left_iiner">First Name</div>
            <div class="student_right_iiner">' .$us['User']['lastname'].  '</div>';
        echo '</div>
        <div class="student_inner_main">
            <div class="student_left_iiner_two">Parent First Name</div>';
            echo '<div class="student_right_iiner">N/A</div>
        </div>
       <div class="student_inner_main">
            <div class="student_left_iiner_two">Parent First Name</div>
            <div class="student_right_iiner">N/A</div>
        </div>
    </div>';
echo ' <div class="student_inner_lesson_right">

        <div class="student_inner_lesson_left">
        <div class="student_iiner_name_text">Lesson</div>

        <div class="student_inner_main">
        <div class="student_left_iiner">Instrument</div>
        <div class="student_right_iiner">Piano</div>
        </div>

        <div class="student_inner_main">
        <div class="student_left_iiner">Skill Level</div>
        <div class="student_right_iiner">Beginner</div>
        </div>
        <div class="student_inner_main">
        <div class="student_left_iiner_two">Preferred Music Style</div>
        <div class="student_right_iiner">Jazz</div>
        </div>
        <div class="student_inner_main">
        <div class="student_left_iiner">Student Rate</div>
        <div class="student_right_iiner">$25.00</div>
        </div>
        <div class="student_inner_main">
        <div class="student_left_iiner">Lesson Day</div>
        <div class="student_right_iiner">Monday </div>
        </div>        
        <div class="student_inner_main">
        <div class="student_left_iiner">Lesson Time</div>
        <div class="student_right_iiner">5:15-5:45pm</div>
        </div>        
        </div>
    </div>
</div>';
echo '<div class="student_inner_lesson_left">
        <div class="student_iiner_name_text">Contact</div>';

        echo '<div class="student_inner_main">
        <div class="student_left_iiner">Phone</div>';
        echo '<div class="student_right_iiner">'  .$us['User']['phone'].     '</div>
        </div>';

       echo '<div class="student_inner_main">
        <div class="student_left_iiner">Email</div>';
        echo '<div class="student_right_iiner">'  .$us['User']['email'].     '</div>';
       echo '</div>
        <div class="clr"></div>

        <div class="student_inner_lesson_left">
        <div class="student_iiner_name_text">Address</div>
        <div class="student_inner_main">
        <div class="student_left_iiner">Street</div>
        <div class="student_right_iiner">Ple Ln.</div>
        </div>';
       echo '<div class="student_inner_main">
        <div class="student_left_iiner">City</div>
        <div class="student_right_iiner">'  .$us['User']['city']. 	  '</div>
        </div>';
      echo '<div class="student_inner_main">
        <div class="student_left_iiner">State</div>
        <div class="student_right_iiner">'  .$us['User']['state']. 	  '</div>
        </div>';


        echo '<div class="student_inner_main">
        <div class="student_left_iiner">Zip</div>
        <div class="student_right_iiner">'  .$us['User']['zip'].   '</div>
        </div>
        </div>
</div></div>';
	...
	function lessonShedule($id=null)    // this function will give the lesson detail based on he id passed to this function
	{
		...
		if($_POST)
		{
			$start1=$_POST['yearfrom'].'-'.$_POST['monthfrom'].'-01';
			$end  =$_POST['yearto'].'-'.$_POST['monthto'].'-30';
			$condition1=array('conditions'=>array('StudentLesson.stid'=>$_POST['id'],'StudentLesson.dateoflesson <=' => $end,'StudentLesson.dateoflesson >=' => $start1));
			$condition2=array('conditions'=>array('StudentLesson.stid'=>$_POST['id'],'StudentLesson.dateoflesson <=' => $end,'StudentLesson.dateoflesson >=' => $start1));
			$condition12=array('conditions'=>array('StudentLesson.stid'=>$id ,'StudentLesson.dateoflesson <=' => $start));
		...
		$futurelesson           = $this->StudentLesson->find('all',$condition12);
			 ...
			 if(!empty($_POST['day']) && !empty($_POST['month']) && !empty($_POST['year']) )
			  {
				  $this->data['MakeupLesson']['tid']  		 = 14;
				  $this->data['MakeupLesson']['stid']		 = 52;	
				  $this->data['MakeupLesson']['dateoflesson']	 = $_POST['day'];
				  $this->data['MakeupLesson']['monthoflesson']   = $_POST['month']; 	
				  $this->data['MakeupLesson']['yearoflesson']    = $_POST['year'];
				  $this->data['MakeupLesson']['credit']    	 = $_POST['credit'];
				  $this->MakeupLesson->save($this->data ,false);
			  	  $condition					 =array('conditions'=>array('stid'=>$id ));
				  $makeup              				 = $this->MakeupLesson->find('all',$condition);	
				 foreach($makeup as  $makeup1) {
       					
				    echo '<div class="table_main_left_inner_two">';
				    echo '<div class="table_left_box"><div class="table_text_date_gry">'.$makeup1['MakeupLesson']['dateoflesson']. '/' .$makeup1['MakeupLesson']['monthoflesson']. '</div></div>';
				    echo '<div class="table_left_box_to"><div class="table_text_date_gry">Lesson</div></div>';
				    echo '<div class="table_left_box_three"><div class="table_text_date_gry">Make Up Lesson</div></div>';
				    echo '<div class="table_left_box_fo"><div class="table_text_date_gry">-1</div></div>';
				    echo '<div class="table_left_box_four"><div class="table_text_date_red"></div></div>';
				    echo '<div class="table_left_boxs"><div class="table_text_date_red"></div></div></div>';
			            }
				die;
			  }
